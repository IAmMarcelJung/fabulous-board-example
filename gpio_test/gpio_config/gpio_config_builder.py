...
def add_config_to_stream(stream: str, config: int, htv_type: int) -> str:
    ...
    elif config == C_MGMT_HIGH_Z_STRONG_0_DISABLE_OUTPUT:
        next_reg_stream = "1000000000011"
    elif config == C_MGMT_HIGH_Z_STRONG_0:
        next_reg_stream = "1000000000001"
    elif config == C_SPECIAL:
...
def compensate_hold_time_violations(
    ...
    :rtype: str
    """
    for gpio in range(NUM_IO):
        if violation_types[gpio] == H_DEPENDENT:
            stream = compensate_dependent_htv(stream, bpos)

        if violation_types[gpio] == H_INDEPENDENT:
...
if __name__ == "__main__":
    ...
    stream_l, stream_h = build_streams(config_l, config_h, violations_l, violations_h)
