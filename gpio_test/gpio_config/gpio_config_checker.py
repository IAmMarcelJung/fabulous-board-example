...
def print_header(gpio):
    ...
    for z in gpio:
        if z[1] == H_INDEPENDENT:
            ...
            print("_", end="")
        io_number = extract_io_number(z[0])
        if io_number >= 10:
...
def extract_io_number(io_name):
    # There should only be one number, so just get the first result
    number = re.findall(r"\d+", io_name)[0]
    number = int(number)
    return number
...
for i in range(len(gpio_l_reg)):
    if not check_stream(gpio_l_reg[i], config_l[i]):
        print(f"{i}: {gpio_l_reg[i]}, {config_l[i]}")
        error = True
